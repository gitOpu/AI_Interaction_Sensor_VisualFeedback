...
namespace FieldOfView
{
    ...
    public class VictimFieldOfView : FovAsst
    {

        public float viewRadius = 3.0f;
        public float attackRadius = 1.0f;

        [Range(0, 360)] public float viewAngle = 140;
        ...
        public bool isDrawVisibleFieldOfView;
        public Color meshRendererColor;

        void Start()
        {
            meshRendererColor = Color.green;
            ...
            //DrawFieldOfView();
            isDrawVisibleFieldOfView = false;
        }
        ...
        void LateUpdate()
        {
            if (isDrawVisibleFieldOfView)
            {
                if (true) DrawFieldOfView();
                else viewMesh.Clear();
            }
        }
        ...
        void FindVisibleTargets()
        {
            ...
            for (int i = 0; i < targetsInViewRadius.Length; i++)
            {
                ...
                if (Vector3.Angle(transform.forward, dirToTarget) < viewAngle / 2 || dstToTarget <= attackRadius)
                {
                    ...
                    {
                        if (visibleTargets.Count == 0) visibleTargets.Add(target); // take single one
        ...
        void DrawFieldOfView()
        {
            ...
            for (int i = 0; i <= stepCount; i++)
            {
                float angle = transform.eulerAngles.y - viewAngle / 2 +
                              stepAngleSize * i; //(transform.eulerAngles.y + stepAngleSize * i) ;
                ...
                if (i > 0)
                {
                    ...
                    {
                        EdgeInfo edge = FindEdge(oldViewCast, newViewCast, currentRadius, edgeResolveIterations,
                            edgeDstThreshold, obstacleMask);
